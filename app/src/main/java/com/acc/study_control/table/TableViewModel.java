...
package com.acc.study_control.table;
...
import com.acc.study_control.models.Cell;
import com.acc.study_control.models.ColumnHeader;
import com.acc.study_control.models.Member;
import com.acc.study_control.models.RowHeader;
...
public class TableViewModel {
    ...
    public TableViewModel(Context context, List<Member> members) {
    ...
    private List<ColumnHeader> getHeaderList() {
        List<ColumnHeader> list = new ArrayList<>();

        ColumnHeader header = new ColumnHeader(String.valueOf("A0"), "Nombre Completo");
        list.add(header);
        for (int i = 1; i <= 20; i++) {
            header = new ColumnHeader("A" + String.valueOf(i + 1), "#" + String.valueOf(i));
            list.add(header);
        }
        header = new ColumnHeader(String.valueOf("A21"), "Fecha de Bautismo");
        list.add(header);
        return list;
    }
    ...
    private List<List<Cell>> getCellListForSortingMember() {
        List<List<Cell>> list = new ArrayList<>();
        for (int i = 0; i < getRowSize(); i++) {
            ...
            for (int j = 0; j < COLUMN_SIZE; j++) {
                Object text = "cell " + j + " " + i;
                String id = j + "-" + i;
                Cell cell = new Cell(id, members.get(i), "");
                cellList.add(cell);
            }
            list.add(cellList);
        }
    ...
    public List<List<Cell>> getCellList() {
        return getCellListForSortingMember();
    }
